...
class TestChatSession(unittest.TestCase):
    ...
    @patch("openAIClient.chatSession.Document")
    ...
    @patch("openAIClient.chatSession.Document", side_effect=Exception("Failed to open DOCX"))
    def test_extractTextFromDocx_exception(self, mock_document):
        # Test extracting text from a DOCX file that raises an exception
        text = self.chatSession.extractTextFromDocx("dummy.docx")
        self.assertEqual(text, "")

    @patch("openAIClient.chatSession.extract_text", return_value="Extracted PDF text")
    ...
    @patch("openAIClient.chatSession.extract_text", side_effect=Exception("Failed to open PDF"))
    ...
    @patch("openAIClient.chatSession.os.path.exists", return_value=True)
    ...
    # @patch("openAIClient.chatSession.initOpenAiClient")
    # @patch("openAIClient.chatSession.os.environ", {})
    @patch.dict('openAIClient.config.os.environ', {}, clear=True)
    ...
    @patch("openAIClient.chatSession.initOpenAiClient")
    @patch("openAIClient.chatSession.os.environ", {"AZURE_OPENAI_API_KEY": "test-api-key"})
    ...
    @patch("openAIClient.chatSession.tiktoken.get_encoding")
